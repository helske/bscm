...
#' @rdname plot_weights
#' @export
plot_weights.bscmfit <- function(x, point_estimate = "median",
                                 order = NULL, coverage = c(0.5, 0.95), 
                                 linewidth = 1, point_size = 2, reverse = NULL,
                                 ...) {
  stopifnot_(
  ...
  for (i in treated) {
    plots[[i]] <- weights |> 
      ...
      weight_plot(
        unit, coverage, point_estimate, donors, linewidth, point_size, reverse
      )
  }
...
#' @rdname plot_weights
#' @export
plot_weights.bscm_ldo <- function(x,
                                  type = "weight",
  ...
  if (identical(type, "rank")) {
    ...
    for (i in treated) {
      plots[[i]] <- rank_plot(
        ...
        donor_order, linewidth, point_size, reverse
      )
    }
  } else {
    ...
    for (i in treated) {
      plots[[i]] <- weight_plot_ldo(
        ...
        coverage, point_estimate, donor_order, linewidth, point_size, 
        reverse
      )
    }
  }
...
rank_plot <- function(weights, donor_order, linewidth, point_size, 
                      reverse = TRUE) {
  removed_donor <- x <- gap <- segment <- x_next <- rank_next <- NULL
  rank_data <- weights |> 
    mutate(
      rank = rank(-mean, ties.method = "first"), .by = removed_donor
    ) |> 
    mutate(
  ...
  point_data <- rank_data |> 
    filter(
      row_number() == 1 | row_number() == n(), .by = c(unit, segment)
    )
  gap_data <- rank_data |>
    mutate(
      x_next = dplyr::lead(x),
  ...
  rank_data |> 
    ...
    geom_point(
      data = point_data, size = point_size
    ) +
    scale_y +
...
weight_plot <- function(x, coverage, point_estimate, donors, 
                        linewidth, point_size, ldo_points = NULL, 
                        reverse = TRUE) {
  ...
  if (!is.null(ldo_points)) p <- p + ldo_points
...
weight_plot_ldo <- function(x, coverage, point_estimate, 
                            donors, linewidth, point_size, reverse) {
  ...
  ldo_points <- geom_point(
    data = ldo,
    aes(x = mean, y = unit),
    shape = 124, size = 1.5 * point_size, alpha = 0.7, colour = "grey50"
  )
  weight_plot(
    base, coverage, point_estimate, donors, linewidth, point_size, 
    ldo_points, reverse
  )
}
