// Bayesian SCM with single treated unit and time-varying intercept and covariates
data {
...
parameters {
  ...
  real a; // intercept
  vector[J] a_z; // intercepts for donors
  vector[K] beta; // regression coefficients
  matrix[D, K + 1] delta; // spline coefficients
  vector<lower = 0>[K + 1] sigma_delta;
}
transformed parameters {
  // to simplex, omega ~ Dirichlet(kappa);
  vector[J] omega = omega_raw / sum(omega_raw);
  // vector[T] mu = spline_matrix * cumulative_sum(sigma_delta[1] * delta[, 1]);
  // matrix[T, K] gamma;
  // for (k in 1:K) {
  //   gamma[, k] = spline_matrix * cumulative_sum(sigma_delta[1 + k] * delta[, 1 + k]);
  // }
   // vector[T] mu = spline_matrix * delta[, 1];
  // matrix[T, K] gamma = spline_matrix * delta[, 2:(K + 1)];
  vector[T] mu = spline_matrix * sigma_delta[1] * delta[, 1];
  matrix[T, K] gamma;
  for (k in 1:K) {
    gamma[, k] = spline_matrix * sigma_delta[k + 1] * delta[, k + 1];
  }
  matrix[T, J] Z_res = Z;
  for (j in 1:J) {
    for (t in 1:T) {
      Z_res[t, j] -= a_z[j] + mu[t] + dot_product(X_z_c[j, t, ], beta + gamma[t, ]');
    }
  }
}
model {
  ...
  for (k in 1:(K + 1)) {
    delta[1, k] ~ normal(0, 1);
    delta[2:D, k] ~ normal(delta[1:(D - 1), k], 1);
      // delta[1, k] ~ normal(0, sigma_delta[k]);
    // delta[2:D, k] ~ normal(delta[1:(D - 1), k], sigma_delta[k]);
  }
  beta ~ normal(pr_mean_coef, pr_sd_coef);
  a_z ~ normal(pr_mean_intercept_z, pr_sd_intercept_z);
  sigma_z ~ exponential(pr_rate_sigma_z);
  for (j in 1:J) {
    Z_res[, j] ~ normal(0, sigma_z[j]);
  }
  ...
  for(t in 1:T_pre) {
    y[t] ~ normal(
      a + mu[t] + dot_product(X_y_c[t, ], beta + gamma[t, ]') + 
      Z_res[t, ] * omega, sigma
      );
  }
}
generated quantities {
  ...
  vector[T] synthetic_mean = alpha + mu + X_y * beta + Z_res * omega;
  vector[J] alpha_z;
