...
data {
  ...
  int<lower=0, upper=8> pr_dist_sigma_gamma;
  ...
  matrix[L, 2] pr_pars_sigma_gamma;
...
parameters {
  ...
  vector<lower=0>[use_gamma ? L : 0] sigma_gamma; // SD of RW1 prior xi
...
model {
  ...
  if (pr_dist_intercept == 1) {
    ...
    a ~ student_t(
      pr_pars_intercept[, 1], pr_pars_intercept[, 2], pr_pars_intercept[, 3]
      );
  }
  // prior for beta
  if (pr_dist_beta == 1) {
    ...
    beta ~ student_t(
      pr_pars_beta[, 1], pr_pars_beta[, 2], pr_pars_beta[, 3]
      );
  }
  // prior for gamma
  if (pr_dist_sigma_gamma == 3) {
    sigma_gamma ~ exponential(pr_pars_sigma_gamma[, 1]);
  } else if (pr_dist_sigma_gamma == 4) {
    sigma_gamma ~ gamma(pr_pars_sigma_gamma[, 1], pr_pars_sigma_gamma[, 2]);
  } else if (pr_dist_sigma_gamma == 6) {
    sigma_gamma ~ normal(0, pr_pars_sigma_gamma[, 1]);
  }
  ...
  if (likelihood) {
    ...
    for (i in 1:N) {
      ...
      if (use_alpha) mu += a[i];
      if (use_beta) mu += cX_y[i, 1:Ti, ] * beta;
      if (use_gamma) mu += rows_dot_product(W_y[i, 1:Ti, ], gamma[1:Ti, ]);
      if (use_ar1) mu[2:Ti] += rho[i] * (y[i, 1:(Ti - 1)] - mu[1:(Ti - 1)]);
