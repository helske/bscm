...
test_that("bscmfit$setup values are correct", {
  ...
  expect_equal(fit1_int$setup$model_type, "bscm_int_nox_none_dir")
  expect_equal(fit1_noint$setup$model_type, "bscm_noint_nox_none_dir")
  expect_equal(fit1_x$setup$model_type, "bscm_int_x_const_dir")
  expect_equal(fitN_xz$setup$model_type, "bscm_int_x_const_dir")
  expect_equal(get_treated(fitN_xz), c("1", "2", "3"))
})

test_that("bscmfit$setup with tv() has correct structure", {
  setup <- fitN_tv$setup
  expect_equal(setup$model_type, "bscm_int_x_varying_dir")
